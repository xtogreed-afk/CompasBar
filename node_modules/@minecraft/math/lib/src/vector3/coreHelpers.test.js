...
describe('Vector2 operations', () => {
    ...
    it('successfully adds vectors and returns a new vector', () => {
        const result = Vector2Utils.add(v1, v2);
        expect(result).toEqual({ x: 5, y: 7 });
        expect(result).not.toBe(v1);
    });
    it('successfully subtracts vectors and returns a new vector', () => {
        const result = Vector2Utils.subtract(v1, v2);
        expect(result).toEqual({ x: -3, y: -3 });
        expect(result).not.toBe(v1);
    });
    it('successfully scales a vector and returns a new vector', () => {
        const result = Vector2Utils.scale(v1, 2);
        expect(result).toEqual({ x: 2, y: 4 });
        expect(result).not.toBe(v1);
    });
    it('successfully computes the dot product of a vector', () => {
        const result = Vector2Utils.dot(v1, v2);
        expect(result).toBe(14);
    });
    it('successfully computes the dot product of a vector with a 0 vector', () => {
        const result = Vector2Utils.dot(v1, { x: 0, y: 0 });
        expect(result).toBe(0);
    });
    it('calculates the magnitude', () => {
        const result = Vector2Utils.magnitude(v1);
        expect(result).toBeCloseTo(2.24, 2);
    });
    it('calculates the distance between two vectors', () => {
        const result = Vector2Utils.distance(v1, v2);
        expect(result).toBeCloseTo(4.24, 2);
    });
    it('computes the floor of the vector', () => {
        const input = { x: 1.33, y: 2.14 };
    ...
    it('normalizes the vector', () => {
        const result = Vector2Utils.normalize(v1);
    ...
    describe('clamp', () => {
        const v = { x: 1, y: 1 };
        const minVec = { x: 0 };
        const maxVec = { x: 2, y: 2.5 };
        it('clamps with defaults (no min or max)', () => {
            const result = Vector2Utils.clamp(v);
            expect(result).toEqual({ x: 1, y: 1 });
        });
        it('clamps properly with both min and max', () => {
            const result = Vector2Utils.clamp(v, { min: minVec, max: maxVec });
            expect(result).toEqual({ x: 1, y: 1 });
        });
        it('clamps with min only', () => {
            const result = Vector2Utils.clamp(v, { min: minVec });
            expect(result).toEqual({ x: 1, y: 1 });
        });
        it('clamps with max only', () => {
            const result = Vector2Utils.clamp(v, { max: maxVec });
            expect(result).toEqual({ x: 1, y: 1 });
        });
        it('clamp with zero vector and positive mins and negative max', () => {
            const vZero = { x: 0, y: 0 };
            const min = { y: 1.5 };
            const max = { x: -2.5 };
            const result = Vector2Utils.clamp(vZero, { min, max });
            expect(result).toEqual({ x: -2.5, y: 1.5 });
        });
        // Test clamp function with large vector
        const vLarge = { x: 1e6, y: 1e6 };
        it('clamp with large vector', () => {
            const result = Vector2Utils.clamp(vLarge, { min: minVec, max: maxVec });
            expect(result).toEqual({ x: 2, y: 2.5 });
        });
    });
    it('calculates the lerp halfway between two vectors', () => {
        const result = Vector2Utils.lerp(v1, v2, 0.5);
        expect(result).toEqual({ x: 2.5, y: 3.5 });
    });
    it('calculates the slerp halfway between two vectors', () => {
        const vecA = { x: 1, y: 0 };
        const vecB = { x: 0, y: -1 };
        const result = Vector2Utils.slerp(vecA, vecB, 0.5);
        expect(result.x).toBeCloseTo(0.7071, 3);
        expect(result.y).toBeCloseTo(-0.7071, 3);
    });
    it('calculates two vectors multiplied together', () => {
        const result = Vector2Utils.multiply(v1, v2);
...
describe('Vectorxz operations', () => {
    ...
    it('successfully adds vectors and returns a new vector', () => {
        const result = VectorXZUtils.add(v1, v2);
        expect(result).toEqual({ x: 5, z: 7 });
        expect(result).not.toBe(v1);
    });
    it('successfully subtracts vectors and returns a new vector', () => {
        const result = VectorXZUtils.subtract(v1, v2);
        expect(result).toEqual({ x: -3, z: -3 });
        expect(result).not.toBe(v1);
    });
    it('successfully scales a vector and returns a new vector', () => {
        const result = VectorXZUtils.scale(v1, 2);
        expect(result).toEqual({ x: 2, z: 4 });
        expect(result).not.toBe(v1);
    });
    it('successfully computes the dot product of a vector', () => {
        const result = VectorXZUtils.dot(v1, v2);
        expect(result).toBe(14);
    });
    it('successfully computes the dot product of a vector with a 0 vector', () => {
        const result = VectorXZUtils.dot(v1, { x: 0, z: 0 });
        expect(result).toBe(0);
    });
    it('calculates the magnitude', () => {
        const result = VectorXZUtils.magnitude(v1);
        expect(result).toBeCloseTo(2.24, 2);
    });
    it('calculates the distance between two vectors', () => {
        const result = VectorXZUtils.distance(v1, v2);
        expect(result).toBeCloseTo(4.24, 2);
    });
    it('computes the floor of the vector', () => {
        const input = { x: 1.33, z: 2.14 };
        const expected = { x: 1, z: 2 };
        expect(VectorXZUtils.floor(input)).toEqual(expected);
    });
    it('computes the floor of negative vectors', () => {
        const input = { x: -1.33, z: -2.14 };
        const expected = { x: -2, z: -3 };
        expect(VectorXZUtils.floor(input)).toEqual(expected);
    });
    it('normalizes the vector', () => {
        const result = VectorXZUtils.normalize(v1);
        expect(result.x).toBeCloseTo(0.45, 2);
    ...
    it('converts a vector to a string with overridden options', () => {
        const vector = { x: 1.23456789, z: 2.99 };
        const expectedString1 = '1.2346|2.9900'; // toFixed performs rounding
    ...
    describe('clamp', () => {
        ...
        const maxVec = { x: 2, z: 2.5 };
        it('clamps with defaults (no min or max)', () => {
            const result = VectorXZUtils.clamp(v);
            expect(result).toEqual({ x: 1, z: 1 });
        });
        it('clamps properly with both min and max', () => {
            const result = VectorXZUtils.clamp(v, { min: minVec, max: maxVec });
            expect(result).toEqual({ x: 1, z: 1 });
        });
        it('clamps with min only', () => {
            const result = VectorXZUtils.clamp(v, { min: minVec });
            expect(result).toEqual({ x: 1, z: 1 });
        });
        it('clamps with max only', () => {
            const result = VectorXZUtils.clamp(v, { max: maxVec });
            expect(result).toEqual({ x: 1, z: 1 });
        });
        it('clamp with zero vector and positive mins and negative max', () => {
            const vZero = { x: 0, z: 0 };
            const min = { z: 1.5 };
            const max = { x: -2.5 };
            const result = VectorXZUtils.clamp(vZero, { min, max });
            expect(result).toEqual({ x: -2.5, z: 1.5 });
        });
        // Test clamp function with large vector
        const vLarge = { x: 1e6, z: 1e6 };
        it('clamp with large vector', () => {
            const result = VectorXZUtils.clamp(vLarge, { min: minVec, max: maxVec });
            expect(result).toEqual({ x: 2, z: 2.5 });
        });
    });
    it('calculates the lerp halfway between two vectors', () => {
        const result = VectorXZUtils.lerp(v1, v2, 0.5);
        expect(result).toEqual({ x: 2.5, z: 3.5 });
    });
    it('calculates the slerp halfway between two vectors', () => {
        const vecA = { x: 1, z: 0 };
        const vecB = { x: 0, z: -1 };
        const result = VectorXZUtils.slerp(vecA, vecB, 0.5);
        expect(result.x).toBeCloseTo(0.7071, 3);
        expect(result.z).toBeCloseTo(-0.7071, 3);
    });
    it('calculates two vectors multiplied together', () => {
        const result = VectorXZUtils.multiply(v1, v2);
